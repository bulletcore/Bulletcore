...
const char * const BITCOIN_CONF_FILENAME = "bullet.conf";
const char * const BITCOIN_PID_FILENAME = "bulletd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bullet" is a composite category enabling all Bullet-related debug output
            if(ptrCategory->count(string("bullet"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BulletCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BulletCore
    // Mac: ~/Library/Application Support/BulletCore
    // Unix: ~/.bulletcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BulletCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BulletCore";
#else
    // Unix
    return pathRet / ".bulletcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bullet.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bullet.conf
