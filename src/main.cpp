...
#if defined(NDEBUG)
# error "Bullet Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight < 400) {
        nSubsidyBase = 1; // anti instamine
    } else {
        nSubsidyBase = 10; // standard reward
    }
	
    CAmount nSubsidy = nSubsidyBase * COIN;

    // yearly decline of production by ~16% per year, projected ~16M coins max by year 2043+.
    for (int i = consensusParams.nSubsidyHalvingInterval; i <= nPrevHeight; i += consensusParams.nSubsidyHalvingInterval) {
        nSubsidy -= nSubsidy/6.25;
    }
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;
                                                                      ...
                                                                      // mainnet:    
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 20; //  55.0% 
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 4)) ret += blockValue / 20; //  60.0% 
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 5)) ret += blockValue / 20; //  65.0% 
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 6)) ret += blockValue / 20; //  70.0% 
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 7)) ret += blockValue / 20; //  75.0% 
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BULLET : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(BULLET): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(BULLET): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // BULLET : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(BULLET): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(BULLET): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Bullet Related Inventory Messages
